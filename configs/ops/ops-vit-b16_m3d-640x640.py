# xtt 9.18 只生成mask，可选保存 vis
_base_ = [
    '../_base_/models/ops_vit-b16.py', 
    '../_base_/datasets/m3d.py',
...
test_evaluator = dict(
    type='IoUMetric',
    iou_metrics=['mIoU'],
    output_dir='work_dirs/ops-vit-b16_m3d-640x640/preds/masks',
    keep_results=True
)
val_evaluator = test_evaluator

# ==================== hooks: 彩图是否保存由 save_vis 控制 ====================
if save_vis:
    # 注册 visualizer + hook，保存 vis
    visualizer = dict(
    ...
    default_hooks = dict(
        visualization=dict(
            ...
            draw=True,
            interval=1,
            out_dir='work_dirs/ops-vit-b16_m3d-640x640/preds/vis_data/vis_image'
        )
    )
else:
    # 不注册 visualizer，完全不画 vis
    default_hooks = dict(
        visualization=dict(
            type='SegVisualizationHook',
            draw=False,
...
param_scheduler = [
    dict(type='PolyLR', eta_min=0.0, power=1.0, begin=0, end=60000, by_epoch=False)
]
